...
def calibrate_gauge(filename,folder_path):
	...
	min_angle = input('Sudut min (sudut dial serendah mungkin) - dalam derajat:') #the lowest possible angle
	max_angle = input('Sudut maks (sudut tertinggi) - dalam derajat:') #highest possible angle
	min_value = input('Nilai minimum:') #usually zero
	max_value = input('Nilai maks:') #maximum reading of the gauge
	units = input('Masukkan unit:')
...
def main():	
	folder_path = input("Nama folder di dalam folder file ini:")
	filename = input("Nama file tanpa ekstensi .jpg:")
	...
	print ("Bacaan saat ini: %s %s" %(("%.2f" % val), units))

if __name__=='__main__':
    main()


#face Detection wajah
import imutils
import numpy as np
import cv2
